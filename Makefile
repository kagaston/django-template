...
build:  ## create the build and runtime images
	@docker build --build-arg DEVEL=no -t $(REPO):$(TAG) -f ./docker/django/Dockerfile .

build-dev:  ## create the dev build and runtime images
	@docker build --build-arg DEVEL=yes -t $(REPO):dev -f ./docker/django/Dockerfile .

compile: requirements/main.in ## compile latest requirements to be built into the docker image
	@docker run --rm -v $(shell pwd)/requirements:/local $(REPO):dev python -m piptools compile --upgrade --no-annotate --no-header --allow-unsafe --generate-hashes --output-file /local/main.txt /local/main.in
	@docker run --rm -v $(shell pwd)/requirements:/local $(REPO):dev python -m piptools compile --upgrade --no-annotate --no-header --allow-unsafe --generate-hashes --output-file /local/tests.txt /local/tests.in
...
superuser:
	@docker exec -it $(REPO)_django_1 python manage.py createsuperuser
